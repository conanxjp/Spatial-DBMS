 ...
 /**
  ...
  public  Tuple()
  {
   ...
   public Tuple(byte [] atuple, int offset, int length)
   {
      ...
      //Reverse the process setHdr() to retrieve fldOffset[], by Peng Zhang
      fldOffset = new short[fldCnt +1];
      int pos = offset + 2; //position of first fldOffset
      try {
          for (int indx = 0; indx < fldCnt + 1; indx++) {
        	  fldOffset[indx] = Convert.getShortValue(pos,data);
        	  pos += 2;
          }
      } catch (IOException e) {
  		// TODO Auto-generated catch block
 ...
 throws IOException, InvalidTypeException, InvalidTupleSizeException
{
  ...
  //write field count information to the byteArray
    ...
    throws IOException
 {
  ...
  for (i=0; i< fldCnt-1; i++)
   ...
   case AttrType.attrString:
     ...
     break;
   case AttrType.attrRect:
	     sval = Convert.getRectValue(fldOffset[i], data).toString();
	     System.out.print(sval);
	     break;  
...
/**
 * Convert this field into a Rect
 *
 * @param    fldNo   the field number
 * @return           the Rect if success
 *
 * @exception   IOException I/O errors
 * @exception   FieldNumberOutOfBoundException Tuple field number out of bound
 */

public Rect getRectFld(int fldNo)
        throws IOException, FieldNumberOutOfBoundException
    {
        ...
        if ( (fldNo > 0) && (fldNo <= fldCnt))
        {
            ...
            Rect rect = Convert.getRectValue(fldOffset[fldNo -1], data);
 ...
 /**
   ...
   public Tuple setRectFld(int fldNo, Rect val)
		  	throws IOException, FieldNumberOutOfBoundException
		  {
		    if ( (fldNo > 0) && (fldNo <= fldCnt))
		     {
			Convert.setRectValue (val, fldOffset[fldNo -1], data);
			return this;
		     }
		    else
		     throw new FieldNumberOutOfBoundException (null, "TUPLE:TUPLE_FLDNO_OUT_OF_BOUND");
...
/*
 * call this method to get the field count of the tuple
 */
