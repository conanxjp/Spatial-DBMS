...
public class IndexScan extends Iterator {
	   ...
	   IOException
  {
	...
	else if (_types[_fldNum -1].attrType == AttrType.attrRect)
	{

	  attrType[0] = new AttrType(AttrType.attrRect);
	  // calculate string size of _fldNum
	  int count = 0;
	  for (int i=0; i<_fldNum; i++) {
	    if (_types[i].attrType == AttrType.attrRect)
	      count ++;
	  }
	  s_sizes[0] = _s_sizes[count-1];
	  ...
	  try
	  {
		  Jtuple.setRectFld(1, ((RectKey)nextentry.key).getKey());
		  //Jtuple.setStrFld(1, ((StringKey)nextentry.key).getKey());
	  }
	  catch (Exception e) {
	    throw new IndexException(e, "IndexScan.java: Heapfile error");
