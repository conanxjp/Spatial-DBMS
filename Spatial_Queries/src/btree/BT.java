...
public class BT  implements GlobalConst{
  ...
  public final static int keyCompare(KeyClass key1, KeyClass key2)
	...
	return  (((IntegerKey)key1).getKey()).intValue()
      ...
      else if ((key1 instanceof RectKey) && (key2 instanceof RectKey))
      {
    	  return (int)(((RectKey)key1).getKey().crdx1 - ((RectKey)key2).getKey().crdx1);
      }
          ...
          throws  KeyNotMatchException,
                             IOException
                  {
                      if ( key instanceof StringKey) {
                          ...
                          return 4;
                      else if (key instanceof RectKey)
                      {
                          return 32;
                      }
                      else throw new KeyNotMatchException(null, "key types do not match");
  ...
  protected final static
	...
	else if (keyType == AttrType.attrRect)
	{
		key = new RectKey(Convert.getRectValue(offset, from));
	}
  ...
  protected final static byte[] getBytesFromEntry( KeyDataEntry entry )
	   ...
	   ConvertException
    {
	  ...
	  Convert.setStrValue( ((StringKey)entry.key).getKey(),
       ...
       else if (entry.key instanceof RectKey)
        {
        	Convert.setRectValue(((RectKey)entry.key).getKey(), 0, data);
        }
