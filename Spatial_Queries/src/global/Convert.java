 ...
 */
        ...
        throws java.io.IOException
 {
  ...
  /**
   * Update Rect data from the given byte array at the specified position
   * convert it to a Rect
   * @param       data            a byte array
   * @param       position        the position in data[]
   * @exception   java.io.IOException I/O errors
   * @return      the character
   */
  public static Rect getRectValue (int position, byte []data)
          throws java.io.IOException
      {
          InputStream in;
          DataInputStream instr;
          byte[] tmp = new byte[32];    // double(8) * 4
          // copy the value from data array out to a tmp byte array
          System.arraycopy (data, position, tmp, 0, 32);
  ...
  // we implements our shape value here

  public static void setRectValue (Rect value, int position, byte []data)
          throws java.io.IOException
      {
          /* creates a new data output stream to write data to
           * underlying output stream
          ...
          // write the value to the output stream
          outstr.writeDouble(value.crdx1);
          outstr.writeDouble(value.crdy1);
          outstr.writeDouble(value.crdx2);
          outstr.writeDouble(value.crdy2);
          // creates a byte array with this output stream size and the
          // valid contents of the buffer have been copied into it
          byte []B = ((ByteArrayOutputStream) out).toByteArray();
